...
class TextInputArchive : public frozen::InputArchive<TextInputArchive> {
 public:
  ...
  void LoadType(std::string& data, const std::string& str) const { data = str; }
  ...
  void LoadType(char& data, const std::string& str) const {
    data = std::stoi(str);
  }
...
template <class T>
requires std::is_fundamental_v<T>
inline void FROZEN_SERIALIZE_FUNCTION_NAME(TextOutputArchive& ar, T const& t) {
  ar.SaveText(std::to_string(t), sizeof(t));
}
...
inline void FROZEN_LOAD_FUNCTION_NAME(TextInputArchive& ar, std::string& bd) {
