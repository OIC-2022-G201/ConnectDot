...
void RenderMof::AddTexture(const ITexturePtr texture, const Vector& position,
  ...
  CGraphicsUtilities::RenderScaleRotateTexture(
      position.x + camera_position_.x, position.y + camera_position_.y, scale.x,
      scale.y, angle, uv, color, alignment, texture);
}

void RenderMof::AddLine(const Vector& position1, const Vector& position2,
                        const Color& color) {
  CGraphicsUtilities::RenderLine(position1.x + camera_position_.x,
                                 position1.y + camera_position_.y,
                                 position2.x + camera_position_.x,
                                 position2.y + camera_position_.y, color);
}

void RenderMof::AddRect(const Rect& rect, const Color& color) {
  CGraphicsUtilities::RenderFillRect(
      rect.Left + camera_position_.x, rect.Top + camera_position_.y,
      rect.Right + camera_position_.x, rect.Bottom + camera_position_.y, color);
}
...
void RenderMof::AddCircle(const Circle& circle, const Color& color) {
  CGraphicsUtilities::RenderFillCircle(circle.Position.x + camera_position_.x,
                                       circle.Position.y + camera_position_.y,
                                       circle.r, color);
}

void RenderMof::AddCircleFrame(const Circle& circle, const Color& color) {
  CGraphicsUtilities::RenderCircle(circle.Position.x + camera_position_.x,
                                   circle.Position.y + camera_position_.y,
                                   circle.r, color);
}
...
void RenderMof::SetCameraPosition(const Vector& position) {
  ...
  camera_position_.x += graphics->GetTargetWidth() / static_cast<float>(2);
  camera_position_.y += graphics->GetTargetHeight() / static_cast<float>(2);
}
}  // namespace base_engine
